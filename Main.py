...
class mywindow(QtWidgets.QMainWindow):
	def __init__(self):
		...
		self.ui.raion.setStyleSheet('''border-radius: 8px;background-color: rgb(200,200,200);''')#изменение скругленности и цвета фона поля ввода
		self.ui.list.setStyleSheet('''border-radius: 10px;background-color: rgb(200,200,200); color:rgb(26,26,26);''')#изменение скругленности, цвета текста и цвета фона списка
		self.ui.find.setStyleSheet('''border-radius: 8px;background-color: rgb(200,200,200);''')#изменение скругленности и цвета фона кнопки поиск
		self.ui.else_1.setStyleSheet('''border-radius: 8px;background-color: rgb(200,200,200);''')  # изменение скругленности и цвета фона кнопки поиск
	...
	def token(self):
		...
		if tokken == "PyQt5.QtCore.QUrl('about:blank')":
			...
			if tokken == "PyQt5.QtCore.QUrl('https://oauth.vk.com/blank.html#error=access_denied&error_reason=user_denied&error_description=User denied your request')":
				self.my_web.load(QUrl("https://oauth.vk.com/authorize?client_id=7080257&redirect_uri=https://oauth.vk.com/blank.html&display=page&v=5.101&scope=photos,audio,video,pages,wall,docs,groups,offline&revoke=1&response_type=token"))#если нажали выход загрузить страницу авторизации
